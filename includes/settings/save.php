...
<?php 
	...
	else
	{
		...
		if ($r)
		{
			...
			$q = 'UPDATE apps SET app_name = "'.$company.'", from_name = "'.$from_name.'", from_email = "'.$from_email.'", reply_to = "'.$reply_to.'", campaign_report_rows = '.$campaign_report_rows.', query_string = "'.$query_string.'", gdpr_only = '.$gdpr_only.', gdpr_only_ar = '.$gdpr_only_ar.', gdpr_options = '.$gdpr_options.' WHERE id = '.get_app_info('restricted_to_app').' AND userID = '.get_app_info('main_userID');
			$r = mysqli_query($mysqli, $q);
			if ($r)
			{
			    echo true; 

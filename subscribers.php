...
<div class="row-fluid">
    ...
    <div class="span10">
    	...
    	<div id="subscribeform" class="modal hide fade" style="width: <?php echo $gdpr_options ? '900' : '516';?>px; margin-left: <?php echo $gdpr_options ? '-450' : '-258';?>px;">
		    ...
		    <p><strong>Ready-to-use subscribe form</strong></p>
	        ...
	        <?php 
		        if(version_compare(PHP_VERSION, '5.3.0') >= 0 && function_exists('openssl_encrypt'))
			        $subscription_form_url = get_app_info('path').'/subscription?f='.short('{"brand":"'.get_app_info('app').'", "list":"'.short($lid).'"}');
		        else
			        $subscription_form_url = get_app_info('path').'/subscription?i='.get_app_info('app').'&l='.short($lid);
		    ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$("#form-code, #form-url").mouseover(function(){
			...
			<br/>
			<p><?php if(!get_app_info('is_sub_user')): echo _('To subscribe users programmatically, use the API');?> → <a href="https://sendy.co/api?app_path=<?php echo get_app_info('path');?>" style="text-decoration: underline;" target="_blank">https://sendy.co/api</a>.<?php endif;?></p>
			</div>
				    ...
				    <div id="gdpr-settings" <?php if(!$gdpr_enabled) echo 'style="display:none;"';?>>
					    ...
					    <script type="text/javascript">
						    $(document).ready(function() {
								...
								{
									$.post("includes/subscribers/subscribe-form.php", { lid: <?php echo $lid;?>, from_email: "<?php echo get_app_data('from_email');?>" },
									  function(data) {
				...
				if($p!=null)
				{
		  		...
		  		if($s=='')
		  		{
		  			if($a=='' && $c=='' && $u=='' && $b=='' && $cp=='' && $g=='')
			  			...
			  			$q = 'SELECT * FROM subscribers WHERE list = '.mysqli_real_escape_string($mysqli, $lid).' AND unsubscribed = 0 AND bounced = 0 AND complaint = 0 AND confirmed = 1 AND gdpr = '.$g.' ORDER BY timestamp DESC LIMIT '.$offset.','.$limit;
				}
				else
				{
					if($a=='' && $c=='' && $u=='' && $b=='' && $cp=='' && $g=='')
						...
						$q = 'SELECT * FROM subscribers WHERE list = '.mysqli_real_escape_string($mysqli, $lid).' AND unsubscribed = 0 AND bounced = 0 AND complaint = 0 AND confirmed = 1 AND gdpr = '.$g.' AND (name LIKE "%'.$s.'%" OR email LIKE "%'.$s.'%" OR custom_fields LIKE "%'.$s.'%") ORDER BY timestamp DESC LIMIT '.$offset.','.$limit;
				}
			  	$r = mysqli_query($mysqli, $q);
...
*/
							...
							$q = 'SELECT SUM( IF( s.timestamp <= UNIX_TIMESTAMP(NOW() - INTERVAL 11 MONTH), 1, 0 ) ),
	 						...
	 						FROM subscribers s WHERE list = '.intval($lid).' AND unsubscribed=0 AND bounced = 0 AND complaint = 0 AND confirmed = 1';
	 
	 			            $r = mysqli_query($mysqli, $q);
			 				if ($r && mysqli_num_rows($r)  > 0)
			 				{
			 				    echo implode(",", mysqli_fetch_array($r, MYSQLI_NUM));
			 				}
			 				else 
			 				{

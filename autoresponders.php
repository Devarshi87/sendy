...
<?php 	
	...
	if ($r && mysqli_num_rows($r) > 0)
	{
	    while($row = mysqli_fetch_array($r))
	    {
			...
			$q3 = 'SELECT apps.smtp_host, apps.smtp_port, apps.smtp_ssl, apps.smtp_username, apps.smtp_password, apps.allocated_quota, apps.current_quota, apps.id, apps.gdpr_only_ar FROM lists, apps WHERE apps.id = lists.app AND lists.id = '.$list;
			    ...
			    while($row = mysqli_fetch_array($r2))
			    {
					...
					if($join_date != '')
					{
						...
						if($time == $join_date_with_tc)
						{
					    	...
					    	$html_treated = str_replace('[unsubscribe]', APP_PATH.'/unsubscribe/'.short($email).'/'.short($list).'/'.short($ares_id).'/a', $html_treated);
